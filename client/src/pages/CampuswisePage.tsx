...
export default function CampuswisePage() {
  ...
  useEffect(() => { api.get("/contribution/campuswise").then(setData).catch((e) => toast.error(e.message)).finally(() => setLoading(false)); }, []);
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((i) => (
                <tr key={i.campus} className="hover:bg-slate-50">
                  <td className="px-5 py-3 font-medium text-slate-800"><Link to={`/app/instructors?campus=${encodeURIComponent(i.campus)}`} className="text-brand-700 hover:underline">{i.campus}</Link></td>
