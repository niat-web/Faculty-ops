...
const SUGGESTIONS_BY_ROLE: Record<string, string[]> = {
  ...
  SENIOR_MANAGER: [
    ...
    "Break down by contribution",
    "How many DSA modules in progress?",
    "How many open tasks do I have?",
  ],
  CAPABILITY_MANAGER: [
    "How many reportees do I have?",
    "My team's average training?",
    "Which of my reportees are at risk?",
    "Show Darwinbox profile for an instructor",
    "Any certification submissions from my team?",
  ],
};
        ...
        >
          <div className="flex shrink-0 items-center justify-between border-b border-slate-100 bg-brand-600 px-4 py-3 text-white">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-[11px] text-white/70">{user.role === "CAPABILITY_MANAGER" ? "Training, Darwinbox, certs — your reportees" : "Training, Darwinbox, certs & more"}</div>
              </div>
            </div>
          ...
          <div ref={scrollRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-slate-50 px-4 py-4" aria-live="polite" aria-relevant="additions">
            {!msgs.length && (
              <div className="space-y-3">
                <div className="rounded-xl bg-white p-3 text-sm text-slate-600 ring-1 ring-slate-100">
                  Hi {(user.name || "").split(" ")[0]} — ask about instructors, training stats, Darwinbox profiles, certifications, requests, or tasks. Answers come from live scoped data.
                </div>
            ...
            {msgs.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`rounded-2xl px-3.5 py-2 text-sm ${m.role === "user" ? "max-w-[85%] whitespace-pre-wrap bg-brand-600 text-white" : "w-full max-w-full bg-white text-slate-700 ring-1 ring-slate-100"}`}>
          ...
          <form onSubmit={(e) => { e.preventDefault(); send(input); }} className="flex shrink-0 items-center gap-2 border-t border-slate-100 bg-white px-3 py-2.5 max-sm:pb-[max(0.625rem,env(safe-area-inset-bottom))]">
            <input
              ...
              placeholder="Ask about instructors, training, certifications…"
              maxLength={1000}
