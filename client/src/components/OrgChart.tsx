...
const initials = (n: string) => (String(n || "").trim().split(/\s+/).map((w) => w[0]).filter(Boolean).slice(0, 2).join("").toUpperCase()) || "?";

interface CM { id: string; name: string; count: number }
interface SM { id: string; name: string; cms: CM[] }
export interface OrgData { totalInstructors: number; totalManagers: number; sms: SM[] }

export default function OrgChart({ data }: { data: OrgData }) {
  const wrapRef = useRef<HTMLDivElement>(null);
  const contentRef = useRef<HTMLDivElement>(null);
  const nodeEls = useRef<Record<string, HTMLElement>>({});
  ...
  const setRef = (id: string) => (el: HTMLElement | null) => { if (el) nodeEls.current[id] = el; };

  // Orthogonal connector paths (left→right, with a shared bus per parent).
  useLayoutEffect(() => {
    const content = contentRef.current;
    if (!content) return;
    const m = (id: string) => { const el = nodeEls.current[id]; return el ? { l: el.offsetLeft, t: el.offsetTop, w: el.offsetWidth, h: el.offsetHeight } : null; };
    const paths: string[] = [];
    const connect = (pid: string, cid: string) => {
  ...
  }, [collapsed, data, measureTick]);

  // Re-measure once web-fonts finish loading (node sizes shift) so connectors stay aligned.
  useEffect(() => {
    (document as any).fonts?.ready?.then(() => setMeasureTick((t) => t + 1)).catch(() => {});
  }, []);
  ...
  function toggleSm(id: string) { setCollapsed((c) => { const n = new Set(c); n.has(id) ? n.delete(id) : n.add(id); return n; }); }
  ...
  function fullscreen() {
    ...
    if (!el) return;
    // Don't set isFull here — the `fullscreenchange` listener is the single source of truth,
    // so the icon won't desync if the request is rejected.
  ...
  const CmNode = ({ cm }: { cm: CM }) => (
    <Link
      ref={setRef(cm.id) as any}
      to={`/app/instructors?managerId=${cm.id}`}
