// RFC 6238 TOTP (no external dependency) — mirrors the Next app's src/lib/totp.js.
...
// Returns the matched 30s counter (for replay protection), or null if invalid.
export function verifyTokenCounter(secret: string, token: string, window = 1): number | null {
  const t = String(token || "").replace(/\D/g, "");
  if (t.length !== 6) return null;
  const counter = Math.floor(Date.now() / 1000 / 30);
  for (let w = -window; w <= window; w++) if (hotp(secret, counter + w) === t) return counter + w;
  return null;
}

export function otpauthURL(secret: string, label: string, issuer = "FacultyOps"): string {
