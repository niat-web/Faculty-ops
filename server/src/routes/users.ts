...
router.post("/", opsOnly, async (req, res) => {
  ...
  if (password) { const i = passwordIssue(password); if (i) return res.status(400).json({ error: i }); }
...
router.delete("/:id", opsOnly, async (req, res) => {
  ...
  if (target.role === Role.SENIOR_MANAGER) { const n = await User.countDocuments({ managerId: target._id }); if (n > 0) return res.status(409).json({ error: `Reassign the ${n} Capability Manager(s) reporting to them first.` }); }
